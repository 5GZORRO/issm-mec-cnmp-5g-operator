...
// Udr is a type to manage k8s objects for Udr 5G
type Udr struct {
...
func NewUdr(operationRunner common.OperationRunner, k8sUtils *common.K8sUtils, client client.Client) *Udr {
	udr := Udr{
	...
	udr.EntityHandlerImpl.AddTransitionFunction("start", udr.Start)
	udr.EntityHandlerImpl.AddTransitionFunction("stop", udr.Stop)

	return &udr
}

// UpdateStatus called to update CR status
func (a Udr) UpdateStatus(opResult controllerutil.OperationResult, instance common.Entity, reconcileErr error) {
	cr := instance.(*v1alpha1.Udr)
...
// Finalize method for Udr.  Executed on uninstall
func (a Udr) Finalize(request reconcile.Request, instance interface{}) {
	// Nothing to do
}

func (a Udr) Create(transition *v1alpha1.Transition) (common.Entity, error) {
	instance := &fivegv1alpha1.Udr{
		ObjectMeta: metav1.ObjectMeta{
			Name:      transition.Spec.Config.ResourceName,
			Namespace: transition.Spec.Config.ResourceNamespace,
		},
		Spec: fivegv1alpha1.UdrSpec{
			Config: &fivegv1alpha1.UdrConfig{
...
func (a Udr) Start(transition *v1alpha1.Transition) (string, error) {
	...
	return a.operationRunner.Start(pod, transition, "udr", "udr")
}

func (a Udr) Stop(transition *v1alpha1.Transition) (string, error) {
	cr, err := a.getCRForTransition(transition)
	if err != nil {
		return "", err
	}
	...
	return a.operationRunner.Stop(pod, transition, "udr", "udr")
}

func (a Udr) CheckReady(transitionCR *fivegv1alpha1.Transition, entity common.Entity) (bool, error) {
	return strings.ToLower(transitionCR.Spec.Config.TransitionName) == "start", nil
}

func (a Udr) GetCR(transitionCR *fivegv1alpha1.Transition) (common.Entity, error) {
	return a.k8sUtils.GetUdr(types.NamespacedName{
		Namespace: transitionCR.Spec.Config.ResourceNamespace,
		Name:      transitionCR.Spec.Config.ResourceName,
	})
}

func (a Udr) IsReconciled(transitionCR *fivegv1alpha1.Transition) (bool, error) {
...
func (a Udr) getCRForTransition(transition *v1alpha1.Transition) (*v1alpha1.Udr, error) {
	cr, err := a.k8sUtils.GetUdr(types.NamespacedName{
		Namespace: transition.Spec.Config.ResourceNamespace,
		Name:      transition.Spec.Config.ResourceName,
	})
	if err != nil {
...
func (a Udr) getPodForCR(cr *v1alpha1.Udr) (*v12.Pod, error) {
