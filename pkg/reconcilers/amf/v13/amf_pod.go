...
func AmfPod(cr *fivegv1alpha1.Amf) *corev1.Pod {
	container := amfContainer(cr)
	init_container := amfInitContainer(cr)

	return &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			//Name: fmt.Sprintf("%s-%s", cr.Name, "amf"),
			Name:      cr.Name,
			...
			Annotations: map[string]string{
				...
				"k8s.v1.cni.cncf.io/networks": fmt.Sprintf("[{\"name\" : \"sbi\"}, {\"name\" : \"ngap\"}]"),
			},
		},
		Spec: corev1.PodSpec{
...
func amfInitContainer(cr *fivegv1alpha1.Amf) *corev1.Container {
...
func amfContainer(cr *fivegv1alpha1.Amf) *corev1.Container {
	return &corev1.Container{
		Name:            "amf",
