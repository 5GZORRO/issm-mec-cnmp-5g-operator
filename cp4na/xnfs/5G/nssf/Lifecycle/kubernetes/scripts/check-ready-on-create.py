def checkReady(keg, props, resultBuilder, log, *args, **kwargs):
    ...
    found, mongo = keg.objects.get('5g.ibm.com/v1alpha1', 'Nssf', name, namespace=namespace)
    if not found:
        return resultBuilder.failed(f'Could not find Nssf {name} in namespace {namespace}')
