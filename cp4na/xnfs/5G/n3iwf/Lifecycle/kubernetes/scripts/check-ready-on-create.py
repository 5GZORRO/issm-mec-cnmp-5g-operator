def checkReady(keg, props, resultBuilder, log, *args, **kwargs):
    found, start = keg.objects.get('5g.ibm.com/v1alpha1', 'Transition', props['system_properties']['resource_subdomain'] + "-create", namespace=props['deployment_location']['properties']['defaultObjectNamespace'])
    if not found:
        return resultBuilder.failed('Could not find Create Transition')

    metadata = start['metadata']
    status = start.get('status', None)
