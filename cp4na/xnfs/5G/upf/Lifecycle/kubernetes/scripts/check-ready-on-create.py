def checkReady(keg, props, resultBuilder, log, *args, **kwargs):
    ...
    found, mongo = keg.objects.get('5g.ibm.com/v1alpha1', 'Upf', name, namespace=namespace)
    if not found:
        return resultBuilder.failed(f'Could not find Upf {name} in namespace {namespace}')
